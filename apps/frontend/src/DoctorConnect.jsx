...
export default function DoctorConnect({ state, dispatch, live = false, busy = false, error = '', onStop, activity }) {
  ...
  useEffect(() => {
    ...
    if (!open) { dialog.current.close(); recognition.current?.abort(); }
  }, [open]);
  useEffect(() => { setAnswer(''); if (dialog.current) dialog.current.scrollTop = 0; }, [state.stage]);
  ...
  function speak() {
    ...
    r.onerror = () => { setListening(false); setSpeechError('Speech was unavailable. Please type your reply.'); };
    r.onresult = event => dispatch({ type: 'reply', text: event.results[0][0].transcript });
    try { r.start(); } catch { setSpeechError('Please use the text box instead.'); }
  }
  ...
  function reply(event) {
    ...
    dispatch(['date', 'time', 'patient'].includes(state.stage)
      ? { type: `set_${state.stage}`, value: answer } : { type: 'reply', text: answer });
  }
  return <dialog ref={dialog} className="care-dialog" aria-labelledby="care-title" onCancel={e => { e.preventDefault(); dispatch({ type: 'close' }); }}>
    ...
    <div className="care-content">
      ...
      {state.stage !== 'handoff' && <form onSubmit={reply} className="care-reply"><label htmlFor="care-answer">{state.stage === 'date' ? 'Preferred date' : state.stage === 'time' ? 'Preferred time (not a confirmed slot)' : state.stage === 'patient' ? 'Patient name' : state.stage === 'search' ? 'Or say “book the second doctor”' : 'Or type your reply'}</label><div><input id="care-answer" type={state.stage === 'date' ? 'date' : state.stage === 'time' ? 'time' : 'text'} autoComplete="off" maxLength={100} value={answer} onChange={e => setAnswer(e.target.value)} required /><button type="submit" disabled={!answer.trim() || listening}>Continue</button></div>{live ? <p><Mic size={18} /> Or say your answer. MediVoice will reply aloud.</p> : <><button className="care-secondary" type="button" disabled={!Speech || listening} onClick={speak}><Mic size={18} />{listening ? 'Listening…' : 'Press to speak'}</button><small>Optional browser speech (English); may use your browser’s speech service. No LiveKit session.</small></>}{speechError && <p role="alert">{speechError}</p>}</form>}
